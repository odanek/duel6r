...
//		player.life = 100;
	...
	// give the human player a moment to visually find himself on the screen
		...
		if(elapsedFrames % 60 == 0){
				uint dsty = target.targetPosy - 0.2;
				PathSegment@ dst = segmentMap[target.targetPosx, dsty];
				uint new_pathDst = dst.id;
				uint new_pathSrc = segmentMap[player.position.x,  player.position.y - 0.2].id;
		...
		if(player.position.x <  target.pathx){
			player.setControl(Right);
		} else {
			player.setControl(Left);
		}
		int itsJumpTime = Rand(100);
		float jumpDiff = target.pathy - player.position.y;
		if( (jumpDiff > 0.5 && jumpDiff < 2.3)|| itsJumpTime == 0){
